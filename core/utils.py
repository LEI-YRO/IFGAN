...
def translate_using_reference(nets, args, x_src,y_src, x_ref, y_ref, filename):
    ...
    # x_concat = [x_src]#Tensor变list
    # for i, s_ref in enumerate(s_ref_list):
    x_fake , x_rec = nets.generator(x_src, y_ref, y_src, masks=masks)
    # x_fake = nets.generator(x_src, y_ref, masks=masks)
    # x_rec = nets.generator(x_src, y_src, masks=masks)
...
def translate_using_onereference(nets, args, x_src,y_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size()
    # wb = torch.ones(1, C, H, W).to(x_src.device)#返回一个内容全为1,size为1×C×H×W的张量
    # x_src_with_wb = torch.cat([wb, x_src], dim=0)#拼接为一个N+1,C,H,W的张量
    print(y_src)
    ...
    for n, x in enumerate(x_src):
        x = x.unsqueeze(0)
        for i, s in enumerate(y_ref):
            s = s.unsqueeze(0)
            l = 1-s
            x_fake ,x_rec= nets.generator(x, s,l, masks=masks)
            # x_fake = nets.generator(x, s, masks=masks)
            filename = ospj(args.result_dir, '%02d_reference.jpg'% (n))
...
def invert_labels(labels):
    """
    将形状为 (batchsize,) 的标签张量中 1 变为 0，0 变为 1。
    
    参数:
    labels (torch.Tensor): 形状为 (batchsize,) 的标签张量。
    
    返回:
    torch.Tensor: 标签值已经被反转的张量。
    """
    if not isinstance(labels, torch.Tensor):
        raise ValueError("输入必须是一个 PyTorch 张量")

    if labels.dim() != 1:
        raise ValueError("输入张量的形状必须是 (batchsize,)")
